...
var list = {
	...
	bind : function() {
		...
		$("#insert").click(function() {
			list.openDialog({
				status:"增加",
				url:basePath + '/admin/operation/addPage'				
			});
		...
		$("#delete").click(function() {
					...
					$.getJSON(basePath + '/admin/operation/delete?ids=' + ids, function(
									data) {
								if(data.code==1){
									layer.msg('删除成功！', {
									icon : 2,
		...
		$("#modify").click(function() {
			...
			list.openDialog({
				status:"修改",
				url:basePath + '/admin/operation/updatePage?id='+ selected.id
	...
	loadTable : function() {
		...
		var opt = {
			target : "#tb",
			url : basePath + "/admin/operation/findtable",
			uniqueId : "id",
			columns : [{
						checkbox : true
					}, {
